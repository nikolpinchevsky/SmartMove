        ...
        RetrofitClient.api.login(request).enqueue(object : Callback<TokenResponse> {
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful && response.body() != null) {
                    val token = response.body()?.accessToken ?: return
                    sessionManager.saveToken(token)
                    createDefaultProjectIfNeeded()
                } else {
                    Toast.makeText(
                        this@LoginActivity,
                        getString(R.string.error_login_failed, response.code()),
                        Toast.LENGTH_LONG
                    ).show()
                }
            }
        ...
        RetrofitClient.api.getActiveProject().enqueue(object : Callback<ActiveProjectResponse> {
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    ...
                    Toast.makeText(
                        this@LoginActivity,
                        getString(R.string.error_check_project),
                        Toast.LENGTH_SHORT
                    ).show()
            ...
            override fun onFailure(call: Call<ActiveProjectResponse>, t: Throwable) {
                val message = when (t) {
                    is SocketTimeoutException -> getString(R.string.error_project_check_timed_out)
                    else -> getString(R.string.error_project_check_failed, t.message)
                }
        ...
        RetrofitClient.api.createProject(request).enqueue(object : Callback<ProjectResponse> {
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    Toast.makeText(
                        this@LoginActivity,
                        getString(R.string.msg_project_created_success),
                        Toast.LENGTH_SHORT
                    ).show()
                } else {
                    Toast.makeText(
                        this@LoginActivity,
                        getString(R.string.error_create_project, response.code()),
                        Toast.LENGTH_LONG
                    ).show()
                }
            ...
            override fun onFailure(call: Call<ProjectResponse>, t: Throwable) {
                val message = when (t) {
                    is SocketTimeoutException -> getString(R.string.error_create_project_timed_out)
                    else -> getString(R.string.error_create_project_failed, t.message)
                }
